#include "CustomSecurityEntity.h"
...
namespace artery {
    Define_Module(CustomSecurityEntity)
int CustomSecurityEntity::numInitStages() const
{
    return 2;
}

void CustomSecurityEntity::initialize(int stage)
{
    if (stage == 0) {
...
void CustomSecurityEntity::finish()
{
...
std::unique_ptr<vs::Backend> CustomSecurityEntity::createBackend(const std::string& name) const
{
...
std::unique_ptr<vs::CertificateProvider> CustomSecurityEntity::createCertificateProvider(const std::string& name) const
{
...
std::unique_ptr<vs::CertificateValidator> CustomSecurityEntity::createCertificateValidator(const std::string& name) const
{
...
vs::SignService CustomSecurityEntity::createSignService(const std::string& name) const
{
...
vs::VerifyService CustomSecurityEntity::createVerifyService(const std::string& name) const
{
...
vs::EncapConfirm CustomSecurityEntity::encapsulate_packet(vs::EncapRequest&& request)
{
    return notNullPtr(mEntity)->encapsulate_packet(std::move(request));
}

vs::DecapConfirm CustomSecurityEntity::decapsulate_packet(vs::DecapRequest&& request)
{
